...
for my $iub (1..99)
{
   my $nodeB = sprintf "SubNetwork=ONRM_RootMo_R,SubNetwork=$rnc,MeContext=${rnc}RBS%02d,ManagedElement=1,NodeBFunction=1", $iub;
   print "NodeB is $nodeB\n";
   my $result = create_mo_CS( mo => $nodeB, plan => $plan);  # create NodeB MO
   print_result($result);

   my $iubLink = "$iubLink_prefix$iub";
   print "Iub Link is $iubLink\n";  
   $result = create_mo_CS( mo => $iubLink, plan => $plan, attributes => "iubLinkNodeBFunction $nodeB rncModuleRef SubNetwork=ONRM_RootMo_R,SubNetwork=$rnc,MeContext=$rnc,ManagedElement=1,RncFunction=1,RncModule=1");  # create IubLink MO
   print_result($result);
